...
# ==================== [ 🔒 HYBRID KEY CHECKER (ONLINE + OFFLINE CACHE) ] ====================
...
def load_offline_cache():
    """Load previously saved key status from local file"""
    global CACHED_USER_STATUS
    if os.path.exists(OFFLINE_CACHE_FILE):
        try:
            with open(OFFLINE_CACHE_FILE, "r") as f:
                data = json.load(f)
                CACHED_USER_STATUS = data.get("status")
                return CACHED_USER_STATUS
        except:
            pass
    return None

def save_offline_cache(status):
    """Save key status to local file for offline use"""
    try:
